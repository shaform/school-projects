...
const int T = 50;  // length of the observations
const int NOB = 6;  // number of different observations
...
static void loadHMM( HMM *hmm, const char *filename, const int iter_num = 0 )
{
    int i, j;
    FILE *fp;
    if (iter_num) {
        ostringstream oss;
        oss << filename << "." << iter_num;
        fp = open_or_die(oss.str().c_str(), "r");
    } else {
        fp = open_or_die( filename, "r");
    }
...
static int load_models( const char *listname, HMM *hmm, const int max_num, const int iter_num = 0 )
{
    ...
    while( fscanf(fp, "%s", filename) == 1 ){
        loadHMM( &hmm[count], filename, iter_num );
        count ++;
